...
'''
脚本作用 ：  景安站
新建一个空间，然后在空间内新建几个文件夹，每个文件夹下有几张图。循环……
循环里的参数，需要根据实际情况修改
...
for k in range(s): #代表从0到s(不包含s)
    ...
    n = random.randint(11, 25)  # 生成一个随机数，作为空间名称的长度
    salt = ''.join(random.sample(string.ascii_letters, n))  # 此处是随机生成长度为6的字符串，6  可以修改
    filename = salt.lower()  # 把生成的字符串转换为小写，赋值给存储空间名。
    # 检测空间是否存储，如果存在直接进入空间内批量上传文件；如果不存在，新建一个空间，然后进入空间内批量上传文件
    try:
        sleep(2)
        driver.find_element_by_link_text(filename).click()
    except:
        driver.find_element_by_link_text(u"新建文件夹").click()
        sleep(2)
        driver.implicitly_wait(3)
        driver.find_element_by_id("nameOfNewFloder").clear()
        driver.find_element_by_id("nameOfNewFloder").send_keys(filename)
        driver.implicitly_wait(3)
