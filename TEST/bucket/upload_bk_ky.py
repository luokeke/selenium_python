...
for k in range(0,s):
    ...
    try:
        sleep(5)
        a = driver.find_element_by_link_text(bucketname)
        a.click()
    except:
    ...
    # 上传多个文件
    for i in range(0, 5):  # 上传10个文件，此处参数，可以修改
        file_dir = 'D:\\Pictures'  # 上传文件路径
    ...
    sleep(5)
    # 点击完成按钮，完成文件上传
    up = driver.find_element_by_xpath("html/body/div[12]/div[3]/div/button[1]")
    up.click()
    sleep(2)
    print "现在返回"
    driver.find_element_by_partial_link_text("返 回").click()
    sleep(5)
